---
title: Policy Engine
description: Deep dive into how Railguard inspects tool calls and enforces security policy.
---

The Policy Engine (`rg-policy` crate) is the heart of Railguard. It takes a tool call and returns a `Verdict`: `Allow`, `Deny`, or `Ask`.
...
First, Railguard checks if the tool name matches any permission pattern:
...
Railguard scans tool input for secrets:
...
For `Bash` tool, Railguard checks the command against dangerous patterns:
...
For `Read`, `Write`, and `Edit` tools, Railguard checks if the path matches protected patterns:
...
For `Bash` (URLs in commands) and `WebFetch` tools, Railguard checks for exfiltration domains:
